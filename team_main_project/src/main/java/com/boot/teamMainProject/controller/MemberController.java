...
public class MemberController {
	...
	public String myPage(Model model, HttpServletRequest request) {
		...
		/** 로그인한 회원 정보로 가입한 모임 테이블 리스트 조회 */
		MemberVO gather = new MemberVO();
		gather = service.gather(vo);
		List<GatheringVO> gatVoList = new ArrayList<GatheringVO>();
		if(gather != null) {
			gatVoList = Gatherservice.detailViewGatNoList(gather);
		}
		/** End 로그인한 회원 정보로 가입한 모임 테이블 리스트 조회 */

		model.addAttribute("join", gather);
		model.addAttribute("gatVoList", gatVoList);
